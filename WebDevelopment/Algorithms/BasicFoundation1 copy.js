// 1) Get 1 to 255 - Write a function that returns an array with all the numbers from 1 to 255.

function arrayNumbers(){
    var arr = [];
    for (var i = 0; i<=255; i++) {
        arr.push(i);
    }
    return arr;
}
console.log(arrayNumbers());

// Get even 1000 - Write a function that would get the sum of all the even numbers from 1 to 1000.
// You may use a modulus operator for this exercise.

function sumofEvenNumbers() {
  var sum = 0;
  for (var i=0; i<=1000; i++) {
    if (i%2 == 0){
      sum = sum + i;
    }
  }
  return sum;
}

console.log(sumofEvenNumbers());

// Sum odd 5000 - Write a function that returns the sum of all the odd numbers from 1 to 5000.

function sumofOddNumbers() {
  var sum = 0;
  for (var i =0; i<=5000; i++){
    if (i%2 !== 0){
      sum = sum + i;
    }
  }
  return sum;
}
console.log(sumofOddNumbers());

// Iterate an array - Write a function that returns the sum of all the values within an array.
// (e.g. [1,2,5] returns 8. [-5,2,5,12] returns 14).

function sumofArray(x){

  var sum = 0;
  for (var i =0; i<x.length; i++) {
        sum = sum + x[i];
  }
return sum;
}

console.log(sumofArray([1,2,5]));
console.log(sumofArray([-5,2,5,12]));



// Find max - Given an array with multiple values, write a function that returns the maximum number in the array.
// (e.g. for [-3,3,5,7] max is 7)

function maxArrayValue(x){
  var max = 0;
  for (var i=0; i<x.length; i++) {
      if (x[i] > max) {
       max = x[i];
    }
  }
  return max;
}
console.log(maxArrayValue([-3,3,5,7]));


//Find average - Given an array with multiple values,
//write a function that returns the average of the values in the array. (e.g. for [1,3,5,7,20] average is 7.2)

function averageofArray(x) {
  var sum = 0;
  for (var i = 0; i<x.length; i++) {
    sum = sum + x[i];
  }
 var avg = sum/x.length;
 return avg;
}
console.log(averageofArray([1,3,5,7,20]));


// Array odd - Write a function that would return an array of all the odd numbers between 1 to 50.
//(ex. [1,3,5, .... , 47,49]). Hint: Use 'push' method.


function arrayOfOddNumbers () {
 var arr = [];
 for (var i=0; i<=50; i++) {
   if (i%2!= 0) {
     arr.push(i);
   }
 }
 return arr;
}

console.log(arrayOfOddNumbers());


// Greater than Y - Given value of Y, write a function that takes an array and returns the number of values that are greater than Y.
// For example if arr = [1, 3, 5, 7] and Y = 3, your function will return 2. (There are two values in the array greater than 3, which are 5, 7).


function greaterValues(x,y) {
  var count = 0;
  for (var i= 0 ; i<x.length; i++) {
    if (x[i] > y)
    {
      count = count + 1;
      //var z = console.log(" There are " + count +  " values in the array greather than " + y + "," + " which are " + x[i]);
    }
  }
  return count;
}

console.log(greaterValues([1, 3, 5, 7], 3));


// Squares - Given an array with multiple values, write a function that replaces each value in the array with the value squared
// by itself. (e.g. [1,5,10,-2] will become [1,25,100,4])

function squareofArray(x){
  for (var i = 0; i<x.length; i++){
    x[i] = x[i] * x[i];
  }
  return x;
}
console.log(squareofArray([1,5,10,-2]));


// Negatives - Given an array with multiple values, write a function that replaces any negative numbers within the array with the value of 0. When the program is done the array should contain no negative values.
// (e.g. [1,5,10,-2] will become [1,5,10,0])

function resetNegativeValuesofArray(x){
  for (var i= 0; i<x.length; i++){
    if (x[i]<0) {
      x[i] = 0;
    }
  }
  return x;
}

console.log(resetNegativeValuesofArray([1,5,10,-2]));


// Max/Min/Avg - Given an array with multiple values, write a function that returns a new array that only contains the maximum, minimum,
// and average values of the original array. (e.g. [1,5,10,-2] will return [10,-2,3.5])


function maxminAvg(x) {
  var arr = []
  var max = 0;
  var min = 0;
  var sum = 0;
  for (var i = 0; i<x.length; i++){
    sum = sum + x[i];
    if (x[i] > max) {
      max = x[i];
    }
    else if ( x[i] < min) {
      min = x[i];
    }
  }
  var avg = sum/x.length;
  arr.push(max);
  arr.push(min);
  arr.push(avg);
  return arr;
}
console.log(maxminAvg([1,5,10,-2]));

// Swap Values - Write a function that will swap the first and last values of any given array. The default minimum length of the array is 2. (e.g. [1,5,10,-2] will become [-2,5,10,1]).

function swapofArrayValues(x){
  var temp = x[x.length-1];
  x[x.length-1]= x[0];
  x[0] = temp;
  return x;
}
console.log(swapofArrayValues([1,5,10,-2]));


// Number to String - Write a function that takes an array of numbers and replaces any negative values within the array with the string 'Dojo'.
// For example if array = [-1,-3,2], your function will return ['Dojo','Dojo',2].

function resetNegativetoString(x){
  for (var i = 0; i< x.length; i++){
    if (x[i]<0){
      x[i] = 'Dojo';
    }
  }
  return x;
}

console.log(resetNegativetoString([-1,-3,2]));
